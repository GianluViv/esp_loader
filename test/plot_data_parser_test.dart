...
void main() {
  test(
    'requires an exact allowed prefix and qualifies compatibility names',
    () {
      expect(parsePlotMeasurements('RPM: M1:1200'), isEmpty);
      expect(parsePlotMeasurements('PLOT_DEBUG: M1:1200'), isEmpty);
      expect(
        parsePlotMeasurements('PLOT: M1:1200', allowedPrefixes: {}),
        isEmpty,
      );
      ...
      expect(values.single.name, 'RPM / M1');
      expect(values.single.value, 1200);
    },
  );
  ...
  test('extracts decimal values and units', () {
    final values = parsePlotMeasurements(
      '[22:19:43.627]  PLOT: T1: 28.56°C, RH1: 52.44%',
    );
  ...
  test('ignores equals fields and nonnumeric colon fields', () {
    final values = parsePlotMeasurements(
      'PLOT: ESP-NOW seq=94 ch=6 state: ready',
    );
